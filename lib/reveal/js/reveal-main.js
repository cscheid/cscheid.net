...
function resetSlide() {
  ...
  document.location = document.location.origin + document.location.pathname + urlFrag;
  window.location.reload();
}
function forceNextSlide() {
  var slideNumber = Number(d3.select("div.slide-number span.slide-number-a").text()) - 1;
  document.location = document.location.origin + document.location.pathname + "#/" + (slideNumber + 1);
  window.location.reload();
}
