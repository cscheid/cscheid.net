...
function setupSVG(div, reader, whichPlot)
{
  ...
  var scene = plot.create(div, window.innerWidth * 0.95, ~~(window.innerWidth * 0.95 / aspectRatio));
...
d3.json("data/iris-linear.dimreader")
  .then(data => {
    ...
    plot1States = setupSVG(d3.select("#main"), reader, 0);

    return d3.json("data/iris-tsne.dimreader");
  }).then(data => {
    var points = data.points.map((d, i) => {
      return {
        p: d.range,
    ...
    plot2States = setupSVG(d3.select("#main-2"), reader, 1);
    ...
    debug.expose(jointStates, "states");
  });
