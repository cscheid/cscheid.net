...
function setupSVG(div, reader)
{
  ...
  var scene = plot.create(div, 500, ~~(500 / aspectRatio)); // ~~(600 * aspectRatio), 600);
  ...
  var hueScale = d3.scaleLinear()
      .domain(d3.extent(points, d => d.metadata.u))
      .range([0, 240]);
  ...
  var pointObject = scene.addPoints(points, {
    color: d => d3.hcl(
      hueScale(d.metadata.u),
      60,
      lumScale(d.metadata.u)),
    x: d => d.p[0],
    y: d => d.p[1],
    r: d => 3
  });
...
function setupDimReader(div, points) {
  var reader = dimReader({
    points: points,
    perturbation: () => selected.perturbation
  });
  return setupSVG(div, reader);
}

function setupValueHeatmap(div, points) {
...
d3.json("/data/swiss-roll-pca.dimreader")
  .then(data => {
    ...
    var index = 0;
    var plot1 = setupDimReader(d3.select("#main"), points);
    var plot2 = setupValueHeatmap(d3.select("#main"), points);
