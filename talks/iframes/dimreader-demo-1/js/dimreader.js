...
function distance2(v1, v2) {
  return linalg.norm2(linalg.sub(v1, v2));
}
...
export function dimReader(options)
{
  ...
  function gaussianKernel(p1, p2) {
    return Math.exp(-linalg.norm2(linalg.sub(p1, p2)) / sigma2);
  }
...
function solveLinearSystem(lst)
{
  ...
  var nCols = gridRows * gridCols; // "-1" because we fix the top-left value at 0
  var lambda = 0.01;
